...
function refreshPage() {
    ...
    //console.log("schedule chart data js ---- refresh page")
...
//  hideMenu();
//  showChart();
    console.log("trigger node detail page refresh");
...
var update_times = 0;
localStorage.setItem("selfCanBack", true);

function setBackTask(status) {
    console.log( "set back task :" + status );
    if (status) {
        update_times = 0;
    }
    ...
    if(write_status_info[nodeId] != "undefined"){
            ...
            console.log( "write_status_info --" + nodeId + "  " + write_status_info[nodeId] );
    }
}

function writestatus_success(data) {
    //console.log("writestatus：   " + data.TranStatus);
...
function get_write_status() {
    update_times++;
    //console.log( "update_times:  " + update_times );
...
//    onRefreshButtonClick();
    //保存后,指令未完成,则不可退出
    localStorage.setItem("selfCanBack", false);
